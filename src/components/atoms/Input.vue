    ...
    <div :class="{
      ...
      <slot name="input">
        <input :id="id" :name="name" :type="type || 'text'" v-bind="$attrs" v-model="value"
               class="block w-full p-1.5 bg-transparent border-none text-sm tracking-normal caret-brand focus:bg-brand/20 text-brand hover:bg-brand/20 placeholder:text-brand/50 autofill:!bg-brand"
               ref="input"/>
      </slot>
